...
export const getForumTopicDetails = async (slug) => {
  const query = gql`
...
export const getForumComments = async (slug) => {
  const query = gql`
    query GetForumComments($slug: String = "$slug") {
...
export const submitComment = async (obj, isForum) => {
  const result = null
  if(isForum) {
    result = await fetch('/api/forumComments', {
      method: 'POST',
      headers: {
